...
function EditNews() {
  ...
  //     `https://localhost:7071/api/News/newsById/${id}`
  //   );
  ...
  //   const newsByIdres = await response.json();
  //   console.log("fetch newsById result", newsByIdres);
  //   return newsByIdres;
  // };
  ...
  const { mutateAsync, isLoading: isMutatingNewsEdit } = useMutation(
    async (data) => {
      const response = await fetch(
        `https://localhost:7071/api/News/editNews/${id}`,
        {
          method: "PUT",
          headers: {
          ...
          <div className="mb-4 flex flex-col gap-6">
            ...
            {errors.title && errors.title.type === "minLength" && (
              <p className="text-red-500 font-bold">
                სათაური უნდა იყოს 10 სიმბოლოზე მეტი
              </p>
            )}
